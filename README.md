...
This script will detect a Canon C100 or C300 camera card structure and transform the original camera card directory structure into a usable AIP. The script will:
...
The script currently detects camera card structure based on directory name. If the supplied camera card directory includes a directory called "STREAM," the script identifies the type of camera as a Canon C100. If the supplied camera card directory includes a directory called "CLIPS001," the script identifies the type of camera as a Canon C300. If the camera card directory contains neither of these subdirectories, the script applies a generic profile that sorts video and audio files into an "objects" directory and all other camera-generated files into a "camera\_metadata" directory.
