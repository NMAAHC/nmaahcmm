...
This script will detect a Canon C100 AVCHD or Canon C300 MXF camera card structure and transform the original camera card directory structure into a usable AIP. The script will:
...
The script currently detects camera card structure based on directory name.

* If the supplied camera card directory includes a directory called "PRIVATE," the script identifies the type of camera as a Canon C100. (The "PRIVATE" folder contains all the camera card's MTS files, hidden within packages called PRIVATE, AVCHD, and BDMV.)
* If the supplied camera card directory includes a directory called "CLIPS001," the script identifies the type of camera as a Canon C300. (The "CLIPS001" folder contains all the camera card's MTS files.)
* If the camera card directory contains neither of these subdirectories, the script applies a generic profile. This profile identifies video and audio files using MediaInfo and sorts them into an "objects" directory; all other camera-generated files are sorted into a "camera\_metadata" directory.

The script creates an AIP with the following structures:
...
```
    ...
    │   └── VIDEOFILE2_ffprobe.xml [etc.]
    ├── camera_metadata
    │   └── [all files generated by camera]
    └── objects
        ├── VIDEOFILE1.ext
        ├── VIDEOFILE2.ext [etc.]
        ├── AUDIOFILE1.ext [etc., if present]
        └── MEDIAID.ext [concatenated video file; extension detremined by first file in sequence]
```
